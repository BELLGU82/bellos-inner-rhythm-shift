  ...
  return <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 bell-breathing-container ${isRTL ? 'rtl' : ''}`}>
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="flex justify-between items-center">
          ...
          <NeumorphButton variant="primary" icon={<Plus size={16} />} className="bell-primary-btn text-gray-400">
        ...
        <NeumorphCard className="animate-fade-in">
          <div className="space-y-4">
            ...
            {activeReminders.map(reminder => <div key={reminder.id} className={`
                  bell-task-card
                ...
                <div className="flex items-center justify-between my-[31px] py-[4px] px-[18px] mx-[12px]">
                  ...
                  <div className="flex items-center gap-2">
                    {reminder.expanded ? <button className="bell-outline-icon" onClick={e => {
                  e.stopPropagation();
                  toggleReminderExpansion(reminder.id);
                }}>
                        <X size={18} className="text-bell-muted hover:text-bell-primary" />
                      </button> : <button className="p-2 rounded-full hover:bg-bell-subtle transition-colors" onClick={e => {
                  e.stopPropagation();
                ...
                {reminder.expanded && <div className="mt-4 pt-4 border-t border-bell-subtle animate-fade-in">
            ...
            {activeReminders.length === 0 && <div className={`text-center py-8 text-bell-muted ${language === 'he' ? 'font-miriam' : 'font-mono'}`}>
                <Clock size={48} className="mx-auto mb-4 opacity-30" />
                <p>{language === 'he' ? 'אין תזכורות להיום' : 'No reminders set for today'}</p>
                <NeumorphButton variant="outlined" icon={<Plus size={16} />} className="mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <NeumorphCard title={language === 'he' ? 'עוגני יום' : 'Daily Anchors'} className="animate-fade-in" style={{
          animationDelay: '0.1s'
        }}>
          ...
          <NeumorphCard title={language === 'he' ? 'תזכורות מיינדסט' : 'Mindset Pings'} className="animate-fade-in" style={{
          animationDelay: '0.2s'
        }}>
            <div className="neumorph-inner p-4 rounded-xl space-y-3">
